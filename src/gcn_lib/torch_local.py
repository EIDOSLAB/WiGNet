...
def window_partition(x, window_size=7, graph_type='local'):
    """
    Args:
        x: (B, C, H, W)
        window_size (int): window size
    ...
    if graph_type == "local":
        windows = einops.rearrange(
            x,
            "b c (numh sh) (numw sw) -> (b numh numw) c sh sw",
            sh=window_size,
            sw=window_size,
        ...
        windows = einops.rearrange(
            x,
            "b c (sh numh) (sw numw) -> (b numh numw) c sh sw",
            sh=window_size,
            sw=window_size,
        )  # (num_windows*B, C, window_size, window_size)
    else:
...
def window_reverse(windows, window_size, H, W, graph_type='local'):
    """
    Args:
    ...
    if graph_type == "global":
        x = einops.rearrange(
            windows,
            "(b numh numw) c sh sw -> b c (sh numh) (sw numw)",
            numh=H // window_size,
            numw=W // window_size,
            sh=window_size,
            sw=window_size,
        ...
        x = einops.rearrange(
            windows,
            "(b numh numw) c sh sw -> b c (numh sh) (numw sw)",
            numh=H // window_size,
            numw=W // window_size,
            sh=window_size,
            sw=window_size,
        ...
        raise ValueError("Invalid graph_type. Choose either 'local' or 'global'.")
