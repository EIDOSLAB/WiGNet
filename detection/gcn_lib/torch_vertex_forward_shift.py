...
class WindowGrapher(nn.Module):
    ...
    def __init__(
        ...
        print('Window Grapher w/out fixed input')
    ...
    def _merge_pos_attn(self, attn_mask, batch_size):
        if attn_mask is None:
            return self.relative_pos

        if self.relative_pos is None:
            print('Should not be here..')
            self.relative_pos = torch.zeros((1, attn_mask.shape[1], attn_mask.shape[2])).to(attn_mask.device)
        
        nW_nGH = attn_mask.shape[0]
        #print(f'Attention mask: {attn_mask.shape}')
        return self.relative_pos.repeat(nW_nGH*batch_size,1,1) + attn_mask.repeat(batch_size, 1, 1) # B, N, N
    ...
    def forward(self, x):
        ...
        # print(f'pre pad: {x.shape}')
        if W % self.windows_size != 0:
            x = F.pad(x, (0, self.windows_size - W % self.windows_size))
        if H % self.windows_size != 0:
            x = F.pad(x, (0, 0, 0, self.windows_size - H % self.windows_size))
        
        # print(f'post pad: {x.shape}')

        _,_, pH, pW = x.shape

        if min(input_resolution) <= self.windows_size:
            # if window size is larger than input resolution, we don't partition windows
            self.shift_size = 0
        ...
        if self.shift_size > 0:
            ...
            img_mask = torch.zeros((1, 1, pH, pW)).to(x.device)
            ...
            if self.adapt_knn:
                # print('Adapting knn!')
                adj_mask = torch.empty((attn_mask.shape[0], attn_mask.shape[1], self.kernel_size)).to(x.device) # nW x N x k
                for w in range(attn_mask.shape[0]):
                    for i in range(attn_mask.shape[1]):
                        all_connection = torch.sum(attn_mask[w,i] == 0)
                        scaled_knn = (self.kernel_size * all_connection) // (self.windows_size * (self.windows_size // 1))
                ...
                adj_mask = adj_mask.repeat(B,1,1)
        ...
        pos_att = self._merge_pos_attn(attn_mask, batch_size=B)
        # pos_att: b*nW, N, N
        ...
        x = window_reverse(x, self.windows_size, H=pH, W=pW)
...
if __name__ == '__main__':
    ...
    sizes = [224*1,224*2,224*3,224*4,224*5]

    for img_size in sizes:

        x = torch.rand((1,48,img_size//4,img_size//4)).to("cuda")
        ...
        if eval_wignet:
            f = open("memory_wignet.txt", "a")
        else:
            f = open("memory_vig.txt", "a")
